...
The <:data:> tag inserts a value into the output. When encountered, Raindrop looks up 'name' in the ViewDataDictionary and writes that object to the output usint its .ToString() method.
...
The <:cond:> tag verifies truthfulness in the following way:
...
The <:ncond:> tag functions identically to the <:cond:> tag, except that it only processes its contents if 'name' is *false*. Truthfulness is identified in the exact same way as <:cond:>.
